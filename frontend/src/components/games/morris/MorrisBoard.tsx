...
function getPhase(pieces: { playerNumber: number; position: number }[], pn: number): 1 | 2 | 3 {
  ...
  const onBoard = pieces.filter(
    (p) => p.playerNumber === pn && p.position >= 0 && p.position <= 23,
  ).length;
  return onBoard === 3 ? 3 : 2;
}

function isInMill(
  pieces: { playerNumber: number; position: number }[],
  pos: number,
  pn: number,
): boolean {
  return MILLS.some(
    (mill) =>
      mill.includes(pos) &&
      mill.every((p) => pieces.some((x) => x.playerNumber === pn && x.position === p)),
  );
}
...
function MorrisBoard({ session, gameState, playerId, isMyTurn, boardOnly, animatingPiece }: MorrisBoardProps) {
  ...
  const occupied = new Set(
    pieces.filter((p) => p.position >= 0 && p.position <= 23).map((p) => p.position),
  );
  ...
  if (isMyTurn && diceRoll === 2) {
    // Removal mode
    const opponentOnBoard = pieces.filter(
      (p) => p.playerNumber === opponentPN && p.position >= 0 && p.position <= 23,
    );
            ...
            >
              ...
              {isValidTarget && !piece && (
                <circle
                  ...
                  cy={cy}
                  r={18}
                  fill="rgba(250,200,50,0.18)"
                  stroke="rgba(250,200,50,0.55)"
                  strokeWidth={1.5}
                >
                  <animate
                    attributeName="r"
      ...
      {!boardOnly && session.players.map((player) => {
        ...
        return (
          <PieceTray
            ...
            unplaced={unplaced}
            captured={captured}
            label={isMe ? 'You' : player.displayName}
            isMe={isMe}
          />
        );
      })}
  ...
  return (
    <div className="flex items-center gap-3 w-full px-1">
      <span
        ...
        style={{ color: isMe ? '#F0E6C8' : '#8A7A60' }}
      >
        {label}
      </span>
