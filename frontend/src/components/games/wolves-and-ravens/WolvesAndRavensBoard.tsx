...
function cellCx(col: number): number {
  return col * CELL + PAD + CELL / 2;
}
function cellCy(row: number): number {
  return row * CELL + PAD + CELL / 2;
}
  ...
  for (const [dr, dc] of DIRECTIONS) {
    for (let dist = 1; dist <= diceRoll; dist++) {
      ...
      for (let step = 1; step < dist; step++) {
        ...
        if (pieces.some((p) => p.position === inter)) {
          blocked = true;
          break;
        }
      }
...
function WolvesAndRavensBoard({ session, gameState, playerId, isMyTurn }: Props) {
  ...
  const wolfThreatened =
    !gameState.finished && blockedNeighbors >= orthNeighbors.length - 1 && orthNeighbors.length > 0;
    ...
    <div className="flex flex-col items-center gap-3">
      ...
      >
        ...
        {Array.from({ length: BOARD_SIZE + 1 }, (_, i) => (
          <g key={`grid-${i}`}>
            <line
              ...
              y1={PAD}
              x2={PAD + i * CELL}
              y2={PAD + BOARD_SIZE * CELL}
              stroke="rgba(40,65,30,0.4)"
              strokeWidth={0.5}
            />
            <line
              x1={PAD}
              y1={PAD + i * CELL}
              x2={PAD + BOARD_SIZE * CELL}
              y2={PAD + i * CELL}
              stroke="rgba(40,65,30,0.4)"
              strokeWidth={0.5}
            />
        ...
        {aliveRavens.map((raven) => {
          ...
          const isWolfTarget =
            isMyTurn && myPN === wolfPN && diceRoll !== null && wolfMoves.has(raven.position);
          return (
            <g
      ...
      {isWolf ? (
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <span
            ...
            style={{ color: capturedCount >= 4 ? '#D4A017' : '#6A5A40' }}
          >
            {capturedCount}/5
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <span
            className="text-xs font-mono"
            style={{ color: aliveRavenCount <= 4 ? '#EF6060' : '#6A5A40' }}
          >
            {aliveRavenCount}/8
          </span>
        </div>
