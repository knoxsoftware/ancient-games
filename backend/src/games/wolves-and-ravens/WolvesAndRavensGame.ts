...
export class WolvesAndRavensGame extends GameEngine {
  ...
  initializeBoard(): BoardState {
    const pieces: PiecePosition[] = [
      { playerNumber: 0, pieceIndex: 0, position: WOLF_START },
  ...
  validateMove(board: BoardState, move: Move, player: Player): boolean {
    ...
    if (playerNumber === 0) {
      ...
      if (board.pieces.some(p => p.playerNumber === 0 && p.position === to)) return false;
  ...
  applyMove(board: BoardState, move: Move): BoardState {
    ...
    if (board.currentTurn === 0) {
      ...
      const capturedRaven = newPieces.find(p => p.playerNumber === 1 && p.position === move.to);
      if (capturedRaven) capturedRaven.position = 99;

      movingPiece.position = move.to;

      return { pieces: newPieces, currentTurn: 1, diceRoll: null, lastMove: move };
    } else {
      ...
      if (remaining <= 0) {
        return { pieces: newPieces, currentTurn: 0, diceRoll: null, lastMove: move };
      }
      return { pieces: newPieces, currentTurn: 1, diceRoll: remaining, lastMove: move };
    }
  }

  checkWinCondition(board: BoardState): number | null {
    // Wolf wins: captures 5 ravens
    const captured = board.pieces.filter(p => p.playerNumber === 1 && p.position === 99).length;
    if (captured >= WOLF_WIN_CAPTURES) return 0;

    // Ravens win: all orthogonal neighbors of wolf are occupied by ravens
    const wolf = board.pieces.find(p => p.playerNumber === 0);
    if (!wolf) return 1;
    ...
    const surrounded = neighbors.every(pos =>
      board.pieces.some(p => p.playerNumber === 1 && p.position === pos)
    );
    if (surrounded) return 1;
    ...
    if (playerNumber === 0) {
      ...
      for (const [dr, dc] of DIRECTIONS) {
        for (let dist = 1; dist <= diceRoll; dist++) {
          ...
          if (!board.pieces.some(p => p.playerNumber === 0 && p.position === to)) {
      ...
      const aliveRavens = board.pieces.filter(
        p => p.playerNumber === 1 && p.position >= 0 && p.position < TOTAL_CELLS
      );
